...
| `replace_symbol_body(file, line, column, content, related_files?, settle_ms?, root?)` | `safe_edit` over a hand-counted range | Replace the symbol enclosing the position with `content`. LSP `documentSymbol` locates the deepest enclosing symbol; replacement covers signature + body. `content` must be the full new definition. Same diagnostic-delta report as `safe_edit`. |
| `insert_before_symbol(file, line, column, content, related_files?, settle_ms?, root?)` | `safe_edit` at a calculated offset | Insert `content` immediately before the start of the enclosing symbol. Caller controls newlines. Same diagnostic-delta report. |
| `insert_after_symbol(file, line, column, content, related_files?, settle_ms?, root?)` | `safe_edit` at a calculated offset | Insert `content` immediately after the end of the enclosing symbol. Caller controls newlines (typically start with `\n\n` for a blank-line separator). Same diagnostic-delta report. |
| `safe_delete_symbol(file, line, column, force?, related_files?, settle_ms?, root?)` | `safe_edit` over a hand-counted range | Delete the enclosing symbol's full definition. Preflights via SCIP: refuses with the call-site list if any non-definition references remain in the latest Ready index (or if no Ready SCIP exists). Pass `force: true` to override — for refactors where call sites are about to be deleted too, or when the index is stale. |
