...
| v0.20     | Job management for aide-spawned processes: every `run_project` / `run_tests` / `install_package` gets an aide-assigned `job_id` and a tracked `ChildHandle` in an `AsyncMutex<HashMap>`. Three new MCP tools — `job_list`, `job_info(id)`, `job_kill(id, signal?)` — operate exclusively on those registered jobs. **Scope gate:** tools take `job_id`, never raw PID, so there is no API path for an agent to signal a process aide did not spawn. Closes the "hung `cargo test` → restart MCP" friction without opening a generic process-control surface. | 📋 planned |
| v0.20.1   | Read-only system process listing: `process_list(filter?)` over the `sysinfo` crate, scoped to the current user's processes, with a name-substring filter and a result cap. Complements v0.20 by letting an agent see non-aide-managed processes (e.g. the running `aide-mcp` binary itself) for diagnostic purposes — read only, no signal / kill. Generic `process_kill(pid)` is explicitly rejected from the roadmap: PID reuse, agent hallucination of PIDs, and scope creep outweigh the value — the Bash escape hatch stays for the rare real case. | 📋 planned |
