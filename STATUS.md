...
| v0.23     | LSP query primitives parity with serena: `lsp_implementations` (`textDocument/implementation`), `lsp_declaration` (`textDocument/declaration`, distinct from definition for forward-declared C/C++ and TS ambient decls), `lsp_type_hierarchy` (`textDocument/prepareTypeHierarchy` + `typeHierarchy/supertypes`/`subtypes` with `direction` arg = supertypes / subtypes / both). `lsp_implementations` and `lsp_declaration` reuse `LocationHit` + the v0.22 `enclosing_symbol` annotation path; `lsp_type_hierarchy` returns a `TypeHierarchyResult { origin, supertypes?, subtypes? }`. Definition/declaration/implementation share a small `goto_position_params` + `goto_response_to_hits` helper to keep the three call sites flat. Closes the obvious LSP query gap surfaced by the serena comparison. | ✅ done |
| v0.24     | Symbolic edit primitives: `replace_symbol_body`, `insert_before_symbol`, `insert_after_symbol`, `safe_delete_symbol`, all keyed by `(file, line, column)` — consistent with the rest of the LSP-side API. Symbol's full range resolved via LSP `documentSymbol` (deepest enclosing symbol wins, falling back through `selection_range` then `range`); `safe_edit`'s body extracted into a shared `apply_and_snapshot` helper so all five edit tools snapshot diagnostics the same way. `safe_delete_symbol` preflights `scip_references`: refuses with the call-site list if any non-definition occurrences remain in the latest Ready SCIP (or if no Ready SCIP exists), with `force: true` opt-out for stale-index / planned-rip scenarios. New `position_to_byte_offset` honours UTF-16 LSP columns for multi-byte source. Returns the same `SafeEditReport` shape as `safe_edit`, enclosing-symbol-annotated via the v0.22 path. Fills the write-side gap the serena comparison flagged. | ✅ done |
| v0.25     | `project_onboard(path?, history_limit?, readme_bytes?)` aggregate — one MCP call returns `{root, languages, branch, head_sha, readme, config_files, recent_commits, indexer_state, indexer_head_sha, project_map, test_entry_points}`. README discovery walks an ordered candidate list (README.md, readme.md, README, README.txt, README.rst, Readme.md) and caps the excerpt at `readme_bytes` (default 2048) at a UTF-8 char boundary; `config_files` probes a hardcoded per-`LanguageId` set (Cargo.toml/rust-toolchain.toml for rust, package.json/tsconfig.json/lockfiles for node, …); `test_entry_points` filters `project_map` symbols through the plugin's `is_test_symbol` heuristic; every sub-query is best-effort — a missing piece becomes null/empty without failing the call. Pure composition, no new backend. | ✅ done |
| v0.26     | Memory tools as MCP: `memory_write`, `memory_read`, `memory_list(filter?)`, `memory_delete`, `memory_rename(old, new)`, `memory_edit(name, pattern, replacement)` (regex replace in body) over `~/.aide/memory/<repo_slug>/`. Each memory is a markdown file with YAML frontmatter (`name` + optional `description`); the format mirrors Claude Code's host-level memory layout so existing memory files port across. New `aide_core::slugify_repo_root` helper lifted out of the indexer worker so SCIP and memory dirs share one slug rule; new `AidePaths::memory(slug)` resolver. New `aide_mcp::memory` module owns frontmatter render / parse and the 6 file-system ops with name-injection guards (no `/`, no `..`, no leading dot). For Claude Code this duplicates host memory; the point is portability for Codex / Cursor / custom agents without a built-in layer. | ✅ done |
| v0.27     | HTTP transport — rmcp 1.5's Streamable HTTP mode behind a `--http :PORT` CLI flag (default stays stdio). `:PORT` shorthand binds 127.0.0.1; full `host:port` also accepted. One `AideServer` is constructed up front and cloned per session via `StreamableHttpService::new`'s factory, so the indexer worker, LSP pool, DAP sessions, and job registry persist across sessions inside the single server process. `LocalSessionManager` keeps in-memory session state; default `allowed_hosts = [localhost, 127.0.0.1, ::1]` blocks DNS-rebinding attacks. Per-call MCP progress notifications flow over the SSE stream. Ctrl-C triggers graceful shutdown via `CancellationToken`. New deps: `axum 0.8` (tokio + http1, no extras), `tokio-util 0.7`, `clap 4` (CLI parser). | ✅ done |
